import { ContaBuilder } from "../../../shared.kernel/builders/CanteenBuilders";
...
export class ContaRepository implements IContaRepository {
  ...
  public async create(id_aluno: number): Promise<Conta> {
    ...
    return new ContaBuilder()
      .setIdAluno(response_database.id_aluno)
      .setIdConta(response_database.id_conta)
      .setDataCriacao(response_database.data_criacao)
      .setNumeroConta(response_database.numero_conta);
  }
  ...
  public async adiciona_produto_item_conta(
    id_produto: number,
    id_itemConta: number
  ): Promise<boolean> {
  ...
  public async get_open_item_conta(
    id_conta: number
  ): Promise<{ id_itemConta: number; valor_total: number } | null> {
